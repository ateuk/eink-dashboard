...
Weather = namedtuple('Weather', 'today hourly')
...
def fetch_weather(weather_app_id):
    ...
    return Weather(today=create_weather(current), hourly=hourly)

def render_to_html(events, weather):
...
events, weather = fetch_data(config[CalendarIds], config[ApiKey], config[WeatherAppId])
html = render_to_html(events, weather)
